...
- feat(scoreboard): S169-10 -- TAB-held scoreboard backported from SHANKPIT, FFA path only. Team-mode (TDM/CTF) branch skipped: shankpit-460's GameMode enum never gained SHANKPIT's post-fork team-mode variants and PlayerState.team_id is never assigned anywhere in this codebase (confirmed by grep) -- porting team UI on top of nonexistent team data would be hollow. FFA path is grounded in real live-tracked kills/deaths. Verified visually via a temporary throwaway build (never touching committed source) screenshotted under Xvfb: panel/border/header/columns render correctly. Real server+bot regression test PASS. Team-mode stays open, flagged not dropped. Apple #13800, commit 4ba4e94. (sess-20260813-2154-dda37e8b)

- feat(audio): S169-09 -- spatial audio backported from SHANKPIT (docs2/NORTHSTAR.md §7). Procedural PCM synthesis engine carried over unchanged, wrapped behind a new SoundEngine vtable interface (play_weapon/play_footstep/play_ambient/shutdown) per §7's own instruction, so a future real-asset backend is a second implementation, not a rewrite. Wired audio_play_weapon into the existing shooting-edge-detection point. 11 new unit tests for the spatial-gain math. Both binaries build clean; real server+bot regression test PASS; client verified live under Xvfb (graceful no-audio-device fallback, not a crash). Apple #13798, commit 9904e04. (sess-20260813-2154-dda37e8b)
